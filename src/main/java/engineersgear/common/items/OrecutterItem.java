...
public class OrecutterItem extends SawbladeItem {
    private static Material[] validMaterials = {Material.PLANT, Material.REPLACEABLE_PLANT, Material.GRASS,
        Material.STONE, Material.GLASS, Material.ICE, Material.ICE_SOLID};
    private static ListNBT enchants = new ListNBT();
    public static ResourceLocation texture = new ResourceLocation("engineersgear:item/orecutter_blade");

    static
    {
        CompoundNBT tag = new CompoundNBT();
        tag.putString("id", "fortune");
        tag.putInt("lvl", 3);
        enchants.add(tag);
    }

    public OrecutterItem(String name, int maxDamage, float sawbladeSpeed, float sawbladeDamage) {
